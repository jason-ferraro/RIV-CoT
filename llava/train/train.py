...
from llava.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX, REF_ID_1_INDEX, REF_ID_2_INDEX, REF_ID_3_INDEX, REF_ID_4_INDEX, REF_ID_5_INDEX
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False,
                    has_ref_ids: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    # DRIVING_THEORY: Add id referring tokens to tokenizer as a special tokens
    if has_ref_ids:
        for i in range(5):
            tokenizer.add_tokens([f"<id_{i+1}>"], special_tokens=True)
    ref_id_1_index = tokenizer.convert_tokens_to_ids("<id_1>")
    ref_id_2_index = tokenizer.convert_tokens_to_ids("<id_2>")
    ref_id_3_index = tokenizer.convert_tokens_to_ids("<id_3>")
    ref_id_4_index = tokenizer.convert_tokens_to_ids("<id_4>")
    ref_id_5_index = tokenizer.convert_tokens_to_ids("<id_5>")
    ...
    for i, source in enumerate(sources):
        ...
        for idx, encode_id in enumerate(input_id):
            ...
            # DRIVING_THEORY: Adding id referring tokens
            if encode_id == image_token_index:
                input_id[idx] = IMAGE_TOKEN_INDEX
            if encode_id == ref_id_1_index:
                input_id[idx] = REF_ID_1_INDEX
            if encode_id == ref_id_2_index:
                input_id[idx] = REF_ID_2_INDEX
            if encode_id == ref_id_3_index:
                input_id[idx] = REF_ID_3_INDEX
            if encode_id == ref_id_4_index:
                input_id[idx] = REF_ID_4_INDEX
            if encode_id == ref_id_5_index:
                input_id[idx] = REF_ID_5_INDEX
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False,
    ...
    if conversation_lib.default_conversation.version == "qwen":
        return preprocess_qwen(sources, tokenizer, has_image=has_image, has_ref_ids=has_ref_ids)
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        has_ref_id = ("ref_id" in self.list_data_dict[i]) # DRIVING THEORY ADDITION
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image, has_ref_ids=has_ref_id)
...
def train(attn_implementation=None):
    ...
    print(f'Fixing seed to {training_args.seed} for reproducibility')
